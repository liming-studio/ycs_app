<template>
  <view class="w-full h-screen pt-navbar relative">
    ...
    <view class="px-24 py-16">
      <view class="text-2xl">设置新密码</view>
      <view class="mt-32">
        <!-- form -->
        <u-form>
          <!-- 手机号 -->
...
export default {
  ...
  methods: {
    /** 输入变化改变按钮禁用状态 **/
    changePhone(val) {
      this.btnDisabled = !(val && this.formData.captcha && this.formData.password && this.formData.password2)
    },
    changeCaptcha(val) {
      this.btnDisabled = !(val && this.formData.phone && this.formData.password && this.formData.password2)
    },
    changePassword(val) {
      this.btnDisabled = !(val && this.formData.phone && this.formData.captcha && this.formData.password2)
    },
    changePassword2(val) {
      this.btnDisabled = !(val && this.formData.phone && this.formData.captcha && this.formData.password)
    },
    /******** 验证码相关 ********/
		codeChange(text) {
			if(text === '重新获取') {
